...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "fetch": lambda self, query, *args, **kwargs: [
                {"ec": "1.1.1.1", "substrate": "Test", "organism": "Human", "temperature": 30.0}
            ],
            "fetchval": lambda self, query, *args, **kwargs: 1,
        }
    ],
...
def test_data_endpoint_with_csv_format(test_client: TestClient, sql_capture):
    ...
    assert response.headers["content-disposition"] == "attachment; filename=oed_data.csv"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "fetch": lambda self, query, *args, **kwargs: [
                {"organism": "Human"}, {"organism": "Mouse"}, {"organism": "Rat"}
            ],
...
def test_metadata_endpoint(test_client: TestClient, sql_capture):
    ...
    response = test_client.get(
        "/api/v1/metadata",
        params={"filterable_column": "organism"}
    )
...
def test_metadata_endpoint_invalid_column(test_client: TestClient):
    ...
    response = test_client.get(
        "/api/v1/metadata",
        params={"filterable_column": "invalid_column"}
    )
